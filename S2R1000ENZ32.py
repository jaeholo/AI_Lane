...
class LSTMEncoder(nn.Module):
    def __init__(self, input_dim=4096, hidden_dim=128, num_layers=4, latent_dims=4):
        ...
        self.wcnn = nn.Sequential(
            ...
            nn.Conv1d(in_channels=16, out_channels=32, kernel_size=32, stride=2, padding=2),
            nn.LeakyReLU(negative_slope=0.01),
            nn.Conv1d(in_channels=32, out_channels=64, kernel_size=32, stride=2, padding=2),
            nn.LeakyReLU(negative_slope=0.01),
            nn.Dropout(p=0.5),
            nn.MaxPool1d(2,2),

            nn.Conv1d(in_channels=64, out_channels=128, kernel_size=32, stride=2, padding=2),
            nn.LeakyReLU(negative_slope=0.01),
...
# vae = torch.load("models/034/1/model/696_30.6777171189865_27.063496311565046_31.855796554054987.pth") # configuration of the model has been changed
vae.to(device)
...
###
# input directory
# sacdir = "F:/34/250nz32npz/checktraining"
#
# #output directory
# resdir = "F:/34/resulttraining"
#
# ## if you want to visualize you can use this
# inspectdir = "F:/34/inspecttraining"
#
...
#
#     data = vae(x).cpu().detach().numpy().reshape(32,32)
#     # print(data)
...
train_dataset = VAEDataset1000ENZ1("F:/34/250nz32npz/train")
dev_dataset = VAEDataset1000ENZ1("F:/34/250nz32npz/dev")
test_dataset = VAEDataset1000ENZ1("F:/34/250nz32npz/test") # 012
...
shutil.copyfile("S2R1000ENZ32.py", os.path.join(basep,"code.txt"))
